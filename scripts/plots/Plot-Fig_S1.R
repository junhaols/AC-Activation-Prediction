# Fig S1: per-fold scatter plots (actual vs. predicted activation) for both hemispheres.
library(ggpubr)
library(tidyverse)
library(svglite)
...
save_dir <- paste0(file.path(PROJECT_ROOT, "papers", "figures", "raw", "Fig_S1"), "/")
...
scatter_plot <- function(df, col_x, col_y, color_point, color_line, fill_se,
  ...
  p <- ggplot(df, aes_string(x = col_x, y = col_y)) +
    ...
    stat_cor(label.y = stat_y_position, r.accuracy = 0.01, size = 10) +
    ...
    scale_y_continuous(breaks = seq(-5, 10, 5), limits = c(-8, 12)) +
    scale_x_continuous(breaks = seq(-5, 10, 5), limits = c(-8, 15))
...
# Loop over hemispheres x folds
file_dir <- paste0(file.path(DATA_DIR, "Fig_S1"), "/")
for (hemi in c("Left", "Right")) {
  for (Fold in paste0("Fold", 0:4)) {
    file <- paste0(file_dir, hemi, "_", Fold, "_concat_vert.csv")
    save_path <- paste0(save_dir, hemi, "_", Fold, "_scatters.pdf")
